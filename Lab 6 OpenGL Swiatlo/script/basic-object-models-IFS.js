...
function uvPyramid(baseSize, height) {
    baseSize = baseSize || 1;
    height = (height || 1) * 0.7; 
    ...
    for (let i = 0; i < n; i++) {
        const v1 = baseVertices[(i + 1) % n];
        const v2 = baseVertices[i];
        const center = [0, 0, 0 + zOffset];

        [center, v1, v2].forEach(v => {
            vertices.push(...v);
    ...
    for (let i = 0; i < n; i++) {
        ...
        const U = [v1[0] - top[0], v1[1] - top[1], v1[2] - top[2]];
        const V = [v2[0] - top[0], v2[1] - top[1], v2[2] - top[2]];
        const N = [
            U[1] * V[2] - U[2] * V[1],
            U[2] * V[0] - U[0] * V[2],
            U[0] * V[1] - U[1] * V[0]
        ...
        const normal = length > 0 ? N.map(c => c / length) : [0, 0, 1];
